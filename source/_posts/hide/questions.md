...
- 多路复用（MultiPlexing）：连接共享，每一个请求都是是用作连接共享机制的。一个请求对应一个id，这样一个连接上可以有多个请求，每个连接的请求可以随机的混杂在一起，接收方可以根据请求的 id将请求再归属到各自不同的服务端请求里面。
    ...
    每个请求都要发起TCP连接-》Keep alive，1.0默认关闭，一次连接多个请求-》管线化，1.1，发送多个请求，等待服务端按顺序响应-》多路复用，2.0，不会阻塞，服务端可以乱序响应。帧是最小数据单位，每个帧回标识出属于哪个流。
...
- 阻塞IO：比如socket的默认方式
- 非阻塞IO：比如socket的NONBLOCK方式，需要轮训
- IO复用：select，poll，epoll
- 信号驱动的IO：当进程发起一个IO操作，会向内核注册一个信号处理函数，然后进程返回不阻塞；当内核数据就绪时会发送一个信号给进程，进程便在信号处理函数中调用IO读取数据。类似**回调**。
- 异步IO：aio

<img src="https://pic2.zhimg.com/80/v2-0a418d4da25432f7954139e28cc8b255_1440w.webp" style="zoom:100%;" />

### 异步IO

`aiocb.h`，由AIO控制块`aiocb`来描述IO操作。

`aio_fsync`函数可以让异步文件同步，强制所有等待队列中的异步操作不再等待。

`aio_error`先检查异步操作完成状态，再`aio_return`获取设置在aiocb中的返回值。

`aio_suspend`等待异步操作完成或超时。

## MySQL

### 日志类型

- redu log（Innodb）：物理格式，保证事务**持久性**，用于**崩溃恢复**，日志记录事务执行后的状态。为了保证一致性，最简单做法是每次事务提交时把所有涉及修改的数据页写入磁盘，但是浪费。实际上**只记录事务对数据页做了哪些修改**。

    包括2部分：内存中的日志缓冲，磁盘上的日志文件。每执行一条DML语句，先写入缓冲，积累一些后写入磁盘，这就是**先写日志再写磁盘**。写入时机可以配置。0，延迟写，每秒写入os buffer并调用fsync，最多丢1秒数据；1，实时写实时刷，每次事务提交都写入os buffer并fsync；2，实时写延迟刷，每次提交都写入os buffer，但是每秒调fsync。日志缓冲的内容需要先复制到os buffer，由内核写入文件。

- bin log（Server层面，每个引擎都有）：**主从复制**和数据恢复，从库利用主库上的binlog进行重播，实现主从同步。事务提交时将sql记录，记录写入性操作，以二进制的形式保存在磁盘中。是**逻辑日志**，通过**追加**写入。

    刷盘时机：0，不强制，由系统决定；1，每次commit都写入磁盘；N，每N个事务写入一次。

    格式：STATEMENT，基于SQL语句的复制；ROW，基于行的复制；MIXED，混合复制，无法使用STSTEMENT的时候使用ROW。

- undo log：事务回滚，保证事务**原子性**。会对DML语句记录反操作，比如对delete记录一条insert，或者对update记录一条复原的update。

### 当前读和快照读

- 当前读：读取到**当前最新版本**，会对数据加锁，防止其他事务修改，是悲观锁。
    - 共享锁的select，排他锁的update、insert、delete，串行化
    - 通过next-key解决幻读
- 快照读：基于MVCC并发控制，读到的不一定是最新的。
    - 通过MVCC解决幻读
    - 不加锁的select（事务级别不是串行)

可重复读会在事务对某条记录第一次快照读时创建快照和视图，之后都用这个。读提交每次快照读都会生成快照和视图。

### MVCC如何实现的

InnoDB用事务版本号，行记录中的隐藏列和Undo Log实现。

<img src="https://p9-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/d2fa1c7ffdf241a09a790a34b4b8c817~tplv-k3u1fbpfcp-zoom-in-crop-mark:4536:0:0:0.image" alt="img" style="zoom:50%;" />
...
- **行记录的隐藏列**
  ...
  - roll_ptr:回滚指针，指向前一个版本的记录。
- **Undo Log**：快照由回滚指针形成的链表串联了起来形成**版本链**，数据行指向操作过这行的日志，每个日志也有回滚指针指向更早的日志。需要回滚时只要沿着回滚指针找到事务ID小于回滚事务的数据行。

事务进行**快照读**会产生**读视图**，通过事务ID的大小判断**可见性**。视图里有当前未提交事务id集合，当前最大事务id，创建视图时未提交的最小id，当前事务id。
...
提交任务后先判断核心线程池是否已满，不满则创建线程。满就检查等待队列是否已满，不满则加入。满则检查线程池是否已满，不满则创建非核心线程，满则按照策略处理。
...
logicClock记录头票轮次，以最后一次为准，忽略比自己轮次早的。投票需要给出自身id和事务id，票数相同时把自己的票归给vote_zxid或者vote_myid更大的。
